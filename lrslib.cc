...
long lrs_read_dat(lrs_dat *Q, int argc, char *argv[]) {
  char name[1000];
  char writemode[2] =
      "w"; /* will be set to "a" (append) for overflow or newstart */
  ...
  *tmpfilename = '\0';
  ...
  if (infilenum > 0 && (lrs_ifp = fopen(argv[infilenum], "r")) ==
                           NULL) /* command line overides stdin   */
  {
    fprintf(stderr, "\n*bad input file name\n");
    return (FALSE);
  }
  ...
  if (argc > 2) /* lrs has commandline arguments for newstart */
  {
    if (!Q->nash) {
      ...
      {
        if (strcmp(argv[i], "-newstart") ==
            0) /* newstart not currently used ... */
        {
          strcpy(writemode, "a");
          Q->newstart = TRUE;
  ...
  if (!Q->mplrs && lrs_ofp != stdout &&
      overflow != 2) /* headers for the output file also */
  {
    char *name;
...

{
  ...
  while (fscanf(lrs_ifp, "%s", name) != EOF) {
    ...
    if (strcmp(name, "maximize") == 0 || strcmp(name, "minimize") == 0) {
      ...
      if (dualperturb &&
          overflow != 2) /* apply a perturbation to objective function */
      {
    ...
    if (strcmp(name, "geometric") == 0) {
      if (hull && !Q->voronoi)
        lrs_warning(Q, "warning",
                    "*geometric - option for H-representation or voronoi only, "
                    "skipped");
      else {
    ...
    if (strcmp(name, "maxcobases") == 0) {
      ...
      if (overflow != 2) {
        if (Q->mplrs) /* taken from control line */
          lrs_warning(
              Q, "warning",
              "*maxcobases option skipped - supplied on control line in mplrs");
        else
    ...
    if (strcmp(name, "truncate") == 0) {
      if (hull) {
        if (overflow != 2)
          lrs_warning(Q, "warning",
                      "*truncate - option for H-representation only, skipped");
      } else {
        Q->truncate = TRUE;
    ...
    if (strcmp(name, "nonnegative") == 0)
      if (overflow != 2)
        lrs_warning(
            Q, "warning",
            "*nonnegative - option must come before begin line - skipped");
    ...
    // 2015.2.9   Estimates will continue until estimate is less than subtree
    // size
...
*/
  /* I wish I understood the above remark. For the time being, do it the easy
   * way for Nash */

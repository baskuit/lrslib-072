                                ...
                                Switches to single precision when possible for greater speed */
{
  ...
  long i;
  static unsigned long long maxspval =
      MAXD; /* Max value for the last digit to guarantee */
...
bigu:
  if (zero(v))
    return;
  if ((i = length(u)) < maxsplen ||
      (i == maxsplen && u[maxsplen - 1] < maxspval))
    goto quickfinish;
  divint(u, v, r);
  normalize(u);
...
bigv:
  ...
  if ((i = length(v)) < maxsplen ||
      (i == maxsplen && v[maxsplen - 1] < maxspval))
    goto quickfinish;
  divint(v, u, r);
  normalize(v);
  goto bigu;
